...
\noindent {\normalsize \sc Tom\'{a}\v{s} Flouri$^1$, Emily Jane McTavish$^{1,2}$, Diego Darriba, Pashalia Kapli, Alexandros Stamatakis,  Mark T.\@ Holder $^{1,3}$}\\
\noindent {\small \it 
...
We initially choose an integer $N \geq h(T) > 1$ and use it to construct a grid of
...
$u$ given its two children $v$ and $w$ are $\ell_{u,v}/(\tau_{\phi(u)} -
\tau_{\phi(v)})$ and $\ell_{u,w}/(\tau_{\phi(u)}-\tau_{\phi(w)})$, where $v$
and $w$ are the two children of $u$.
