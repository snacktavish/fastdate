...
\FOR{each leaf node, $u$, in postorder}
    ...
    \ENDFOR
    \STATE \subtreeAgeSum{\minbin{u}}{0} = 1.0
\ENDFOR \\
\COMMENT{\textsc{End of Initialization}}
\FOR{each non-root, internal node $u$ in postorder}
    ...
    \FOR{$d \in [\minbin{u}, 2, \ldots, \numAges)$ }
        ...
        \FOR{$d_v \in [\minbin{v}, 2, \ldots, d -1)$ }
            \STATE $q_v = g\left(\frac{l_v}{d-d_v}\right)$
            \FOR{$d_w \in [\minbin{w}, 2, \ldots, d - 1)$ }
                \STATE $q_w = g\left(\frac{l_w}{d-d_w}\right)$
                \STATE \STATE $\subtreeAgeSum{u}{d} \leftarrow \subtreeAgeSum{u}{d} + q_v q_w \timePriorDensity(d | d_v, d_w)\subtreeAgeSum{v}{d_v}\subtreeAgeSum{w}{d_w}$
            \ENDFOR
        \ENDFOR
    \ENDFOR
\ENDFOR
