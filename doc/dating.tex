...
\begin{eqnarray}
    \ImpDensity(\contTimeVec \mid \dataMatrix) & = &  \Pr(\timeBinRealizationVec \mid \dataMatrix)\prod_{i=\numLeaves}^{2*\numLeaves-1} p(\contTime_i \mid \binFor{i} = \timeBinRealization{i}) \\
    \Pr(\timeBinRealizationVec \mid \dataMatrix) & = &\left[\Pr(D\mid \timeBinRealizationVec)\Pr(\timeBinRealizationVec) \right]/\norm\\
    \norm & = & \Pr(\dataMatrix) = \sum_{\timeBinRealizationVec} \left[\Pr(D\mid \timeBinRealizationVec)\Pr(\timeBinRealizationVec) \right]
