...
The time for tip $x$ is known to be $\timeBin{x}$. 
If all of the tips are extant then $\timeBin{x}=0$ of each tip.
The base case of the recursion is, for any tip $x$, $\subtreeAgeSum{x}{\timeBin{x}} = 1$ and $\subtreeAgeSum{x}{y} = 1$
where $y\neq\timeBin{x}$.
